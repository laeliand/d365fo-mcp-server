...
export const D365FO_FILE_PARAM_SPECS: Record<string, { type: string; description: string }> = {
  ...
  dataField: {
    type: 'string',
    description:
      'add-field on a data-entity or data-entity-extension: source table field name for the mapped field ' +
      ...
      'Omit both dataField and dataSource on a plain data-entity to add an UNMAPPED field instead — ' +
      'fieldType/fieldBaseType or fieldEnumType, optionally computedFieldMethod.',
  },
  dataSource: {
    type: 'string',
    description:
      'add-field on a data-entity or data-entity-extension: source data-source/table name on the entity ' +
      'for the mapped field (e.g. "MyTable"). Required alongside dataField.',
  },
  computedFieldMethod: {
    type: 'string',
    description:
      'add-field on a plain data-entity, with dataField/dataSource both omitted: name of an X++ method on ' +
      'the entity\'s own SourceCode that this unmapped field delegates to for its value — writes ' +
      '<ComputedFieldMethod>. Omitting it produces a bare placeholder field instead, populated by a ' +
      'postLoad(Common _common) override added separately (add-method, objectType="data-entity"). The ' +
      'paired method named here is NOT generated by this op — add it yourself (add-method / ' +
      'add-table-method) returning SysComputedColumn::returnField(tableStr(...), ' +
      'dataEntityDataSourceStr(...), fieldStr(...)) for the SQL-computed pattern shipped entities use ' +
      '(e.g. SysAttachmentSyncEntity.FileName / defineFileName).',
  },
  fields: {
  ...
  propertyPath: {
    ...
    description:
      ...
      '    data-entity (AxDataEntityView): Label, DeveloperDocumentation, PrimaryKey, IsPublic, ' +
...
export const D365FO_FILE_OP_SPECS: Record<string, D365FileOpSpec> = {
  ...
  'add-field': {
    required: ['fieldName'],
    optional: ['fieldType', 'fieldBaseType', 'fieldEnumType', 'fieldMandatory', 'fieldLabel', 'dataField', 'dataSource', 'fieldGroupName', 'computedFieldMethod', 'autoCorrect'],
    ...
    note:
      ...
      'Table/table-extension: otherwise fieldType (EDT) is REQUIRED. ' +
      'data-entity-extension: pass dataField AND dataSource instead — BOTH, or nothing is written; a mapped ' +
      'field has no EDT of its own, it points at dataField on the entity data source dataSource. ' +
      'fieldGroupName is optional and only applies to a data-entity-extension: it appends the field to ' +
      'that BASE-entity field group (shipped extensions use AutoReport). It is not defaulted — a group the ' +
      'base entity does not have is a compile error. ' +
      'objectType="data-entity" (a PLAIN entity, not an extension) supports BOTH field shapes: pass ' +
      'dataField AND dataSource for a mapped field (same AxDataEntityViewMappedField shape as the ' +
      'extension case, straight on the entity\'s own <Fields>), or omit both and pass fieldType/' +
      'fieldBaseType or fieldEnumType for an UNMAPPED field (AxDataEntityViewUnmappedField* — a virtual ' +
      'column with no backing table field). An unmapped field with no computedFieldMethod is a bare ' +
      'placeholder populated by a postLoad(Common _common) override you add separately (add-method, ' +
      'objectType="data-entity"). Passing computedFieldMethod instead writes <ComputedFieldMethod> ' +
      'naming a method on the entity\'s own SourceCode that returns ' +
      'SysComputedColumn::returnField(tableStr(...), dataEntityDataSourceStr(...), fieldStr(...)) — add ' +
      'that method too (add-method / add-table-method); it is not generated by this op.',
  },
  'modify-field': {
    required: ['fieldName'],
