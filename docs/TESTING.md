...
Contract tests worth knowing: `tests/utils/toolInventory.test.ts` asserts the published tool count (60) and keeps `mcpServer.ts`, the startup catalog and `LOCAL_TOOLS` in sync — it fails when a new tool is registered incompletely (see [NEW_TOOL_CHECKLIST.md](NEW_TOOL_CHECKLIST.md)).
