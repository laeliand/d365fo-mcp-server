...
The project uses [Vitest](https://vitest.dev/). 750+ tests run without a live D365FO environment — all external dependencies (SQLite, filesystem, bridge, cache) are mocked.
...
Contract tests worth knowing: `tests/utils/toolInventory.test.ts` asserts the published tool count (61) and keeps `mcpServer.ts`, the startup catalog and `LOCAL_TOOLS` in sync — it fails when a new tool is registered incompletely (see [NEW_TOOL_CHECKLIST.md](NEW_TOOL_CHECKLIST.md)).
